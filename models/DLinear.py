...
class Model(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        if self.task_name == 'classification':
            ...
            self.seq_len = configs.early_cycle_threshold * configs.charge_discharge_length
        else:
    ...
    def classification(self, x_enc, return_embedding):
        ...
        embedding = enc_out.reshape(enc_out.shape[0], -1)
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, x_mark_enc=None, return_embedding=False):
        ...
        if return_embedding:
            dec_out, embedding = self.classification(x_enc, return_embedding)
            return dec_out, embedding  # [B, N]
        else:
            dec_out = self.classification(x_enc, return_embedding)
            return dec_out  # [B, N]
